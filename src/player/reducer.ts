...
export const initialPlayerState: PlayerState = {
  cursor: -1, status: 'idle', speed: 1, traceLength: 0,
}
...
export function playerReducer(s: PlayerState, a: PlayerAction): PlayerState {
  switch (a.type) {
    ...
    case 'pause': return { ...s, status: 'paused' }
    case 'stepForward': {
      const cursor = clamp(s.cursor + 1, s.traceLength)
      return a.auto ? { ...s, cursor } : { ...s, cursor, status: 'paused' }
    }
    case 'stepBack': return { ...s, cursor: clamp(s.cursor - 1, s.traceLength), status: 'paused' }
    case 'seek': return { ...s, cursor: clamp(a.index, s.traceLength) }
    case 'setSpeed': return { ...s, speed: a.speed }
    case 'traceGrew': return { ...s, traceLength: a.length }
    case 'goLive': return { ...s, cursor: s.traceLength - 1, status: 'playing' }
