...
test('initial state', () => {
  expect(init).toEqual({ cursor: -1, status: 'idle', speed: 1, traceLength: 0 })
})

test('play/pause toggle status', () => {
...
test('manual step pauses; auto step keeps playing', () => {
  const playing = r(grown, { type: 'play' })
  const manual = r(playing, { type: 'stepForward' })
  expect(manual).toMatchObject({ cursor: 0, status: 'paused' })
  const auto = r(playing, { type: 'stepForward', auto: true })
  expect(auto).toMatchObject({ cursor: 0, status: 'playing' })
})
...
test('seek clamps', () => {
  expect(r(grown, { type: 'seek', index: 99 }).cursor).toBe(4)
  expect(r(grown, { type: 'seek', index: -3 }).cursor).toBe(0)
})

test('traceGrew only updates length', () => {
  const s = r({ ...grown, cursor: 2 }, { type: 'traceGrew', length: 9 })
  expect(s).toMatchObject({ cursor: 2, traceLength: 9 })
})

test('goLive jumps to frontier and plays', () => {
  const s = r({ ...grown, cursor: 1, status: 'paused' as const }, { type: 'goLive' })
  expect(s).toMatchObject({ cursor: 4, status: 'playing' })
})

test('reset keeps speed, starts playing from -1', () => {
  const fast = r(grown, { type: 'setSpeed', speed: 4 })
  expect(r(fast, { type: 'reset' })).toEqual({ cursor: -1, status: 'playing', speed: 4, traceLength: 0 })
})
