...
- [ ] **Grid side-by-side compare** — both runs' 270-head thumbnail grids
...
- [ ] **Probability-delta view** — computed diff at the selected cycle:
...
- [ ] **>2-run comparison** — generalize A/B to N columns. High UI cost;
  most pedagogy lives in pairwise contrasts — deliberately parked.
  *(M3 spec, out of scope)*

## Attention / interpretability depth

- [ ] **Per-cycle grid evolution** — make the head-explorer grid scrubbable
...
- [ ] **Coreference-head discovery** — real mode detects previous-token /
...
- [ ] **Chat templating** — run prompts through SmolLM2-Instruct's actual
...
- [ ] **Storage-consistency follow-up** — two parked M3 minors together:
  ...
  *(M3 final review)*
- [ ] **Polish sweep over parked minors** — `aria-label`s for the
...
- [ ] **Mobile layout** — the app is desktop-first; a narrow-viewport pass
  is substantial design work for an audience that mostly explores on
  laptops. *(v1 spec, out of scope)*
- [ ] **npm-audit recheck** — 4 high transitive CVEs via
